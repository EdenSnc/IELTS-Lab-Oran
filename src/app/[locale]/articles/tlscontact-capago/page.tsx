    ...
    <>
      ...
      <article className={`pt-32 pb-32 px-6 max-w-3xl mx-auto ${isRtl ? 'font-cairo' : ''}`} dir={isRtl ? 'rtl' : 'ltr'}>
          ...
          <div className="prose prose-lg prose-gray max-w-none text-gray-700 leading-relaxed">
              ...
              {/* Visual Infographic: Requirements Checklist */}
              <div className="bg-white border border-gray-200 rounded-2xl shadow-sm p-6 my-8 not-prose">
                <h3 className="font-bold text-lg text-charcoal mb-4 flex items-center gap-2">
                  ...
                  {locale === 'ar' ? 'متطلبات Campus France' : locale === 'fr' ? 'Exigences Campus France' : 'Campus France Requirements'}
                </h3>
                <ul className="space-y-3">
                    {francePoints.map((p, i) => (
                      <li key={i} className="flex items-start gap-3">
                        <div className="w-5 h-5 rounded bg-green-100 text-green-600 flex items-center justify-center shrink-0 mt-0.5">
                          <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" />
                          </svg>
                        </div>
                        <span className="text-gray-700 text-sm leading-relaxed" dangerouslySetInnerHTML={{ __html: p }} />
                      </li>
                    ))}
                </ul>
              </div>
